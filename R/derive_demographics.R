...
for (i in 1:100){ long
  ...
  output[i,c("SpeciesAuthor","SpeciesAccepted","Family","Class","Kingdom","Authors","Journal","YearPublication","Country","Continent","Ecoregion","Lat","Lon","StartYear","StartSeason","StartMonth","EndYear","EndSeason","EndMonth","Population","Treatment","MatrixComposite")]=
    ...
    lifeTable=makeLifeTable(matU,matF,matC,notProp,1000)[1:QSD[1],]
    lx=lifeTable$lx
    mx=lifeTable$mx

    #Keyfitz' entropy
    output$H[i]=as.numeric(-t(lx[is.na(lx)==FALSE])%*%log(lx[is.na(lx)==FALSE])/sum(lx[is.na(lx)==FALSE]))
    ...
    if (sum(matF)>0)
    {r=log(Lambda)
    limGo=min(length(mx[is.na(mx)==FALSE]),length(lx[is.na(lx)==FALSE]))
    lxmx=lx[1:limGo]*mx[1:limGo]
     lxmx=lx[is.na(lx)==FALSE]*mx[is.na(mx)==FALSE]
    for (r in 1:limGo) {
      if (lxmx[r]==0) {lxmx[r]<-1}
    }
    loglxmx=log(lxmx)
